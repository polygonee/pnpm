...
import _unpackToCafs from '@pnpm/unpack-to-cafs'
...
console.log(cafsDir)
const unpackToCafs = _unpackToCafs.bind(cafsDir, new Map())
...
test('fail when tarball size does not match content-length', async t => {
  ...
  try {
    await fetch.tarball(resolution, {
...
test('redownload the tarball when the one in cache does not satisfy integrity', async t => {
  ...
  const { tempLocation } = await fetch.tarball(resolution, {
    cachedTarballLocation,
    lockfileDir: process.cwd(),
    unpackToCafs,
  })
...
test('retry when tarball size does not match content-length', async t => {
  ...
  const result = await fetch.tarball(resolution, {
    cachedTarballLocation,
    lockfileDir: process.cwd(),
  })
...
test("don't fail when integrity check of local file succeeds", async (t) => {
  ...
  const { filesIndex, tempLocation } = await fetch.tarball(resolution, {
    cachedTarballLocation,
    lockfileDir: process.cwd(),
  })
...
test('retry on server error', async t => {
  ...
  const index = await fetch.tarball(resolution, {
    cachedTarballLocation,
    lockfileDir: process.cwd(),
  })
